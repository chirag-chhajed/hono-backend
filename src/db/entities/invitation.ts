...
const InvitationEntity = new Entity({
  ...
  attributes: {
    invitationId: { type: "string", required: true,default: () => nanoid(32) },
  ...
  indexes: {
    primary: {
      ...
      sk: { field: "sk", composite: ["invitationId"], template: "INVITATION#${invitationId}" },
    },
    byCode: {
      index: "gsi1",
      pk: { field: "gsi1pk", composite: ["code"], template: "CODE#${code}" },
      sk: { field: "gsi1sk", composite: ["invitationId"], template: "INVITATION#${invitationId}" },
    },
  },
},{
    client: dynamoClient,
